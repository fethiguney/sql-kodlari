...
-- satılan meyve türüne (urun_adi) göre satılan 
--(urun_miktari )MIN ve MAX urun miktarlarini, 
...
-- kisi ismine ve urun adına göre satılan ürünlerin 
--toplamını gruplandıran ve isime göre ters sırada listeyen sorguyu yazınız.
...
round(avg(yas)) as yas_ortalamasi from personel
group by ulke, cinsiyet

-- Her ulke için bay ve bayan çalışan sayısı 
--ve yaş ortalamasını, maası 30000 den büyük olanları sorgulayınız.
select ulke, cinsiyet, count(*) as calisan_sayisi,
round(avg(yas)) as yas_ortalamasi  from personel
where maas>30000
group by ulke, cinsiyet
...
select ulke, cinsiyet, count(*) as calisan_sayisi,
round(avg(yas)) as ortalama_yas from personel
where maas>30000 
group by ulke, cinsiyet
